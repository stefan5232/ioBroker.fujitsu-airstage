...
class FujitsuAirstage extends utils.Adapter {
      ...
      if (
        ...
        if (data.iu_indoor_tmp !== undefined) {
          await this.setState(
            ...
            (parseInt(String(data.iu_indoor_tmp)) - 5000) / 100,
            true,
          );
        }
        if (data.iu_outdoor_tmp !== undefined) {
          await this.setState(
            ...
            (parseInt(String(data.iu_outdoor_tmp)) - 5000) / 100,
            true,
          );
        }
    ...
    switch (command) {
      ...
      case "target_temperature": {
        ...
        if (temp >= 16 && temp <= 30) {
          ...
          throw new Error(
            `Invalid temperature: ${temp}. Must be between 16-30°C`,
          );
        }
        break;
      }
  ...
  private startPolling(): void {
    ...
    this.log.info(
      `Started polling with ${interval / 1000}s interval for ${this.devices.length} device(s)`,
    );
  }
