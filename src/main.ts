...
class FujitsuAirstage extends utils.Adapter {
    ...
    private async fetchDeviceModel(device: AirstageDevice): Promise<void> {
        ...
        try {
            const response: AxiosResponse<ApiResponse> = await axios.post(`${device.baseUrl}/GetParam`, payload, {
                headers: {
            ...
            if (response.data && response.data.result === 'OK' && response.data.value) {
                ...
                if (data.iu_model !== undefined && data.iu_model !== null) {
                    await this.setState(`${device.deviceId}.model`, String(data.iu_model), true);
